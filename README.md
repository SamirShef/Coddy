...
> [!IMPORTANT]
> Тип переменной, используемой в выражении, должен соответствовать требуемому типу выражения.
Пример
```
let a : int = 10;
let b : int = a;  // ОК
...
> [!IMPORTANT]
> Тип перечисляемых аргументов и ожидаемых аргументов функции должны быть явно или неявно равны.
...
### Встроенные функции
* `println(message)` - функция, которая выводит любое `message` в консоль
* `input()` - функция, которая возвращает введённое пользователем в консоль сообщения
* `to_int(strNumber)` - функция, которая возвращает конвертированное переданное строковое представление целого числа в целое число.
* `to_float(strNumber)` - функция, которая возвращает конвертированное переданное строковое представление числа с плавающей в число с плавающей точкой.
* `to_double(strNumber)` - функция, которая возвращает конвертированное переданное строковое представление числа с плавающей точкой двойной точности в число с плавающей точкой двойной точности.
* `to_decimal(strNumber)` - функция, которая возвращает конвертированное переданное строковое представление вещественного числа в вещественное число.
...
# ООП (упрощённое)
`Coddy` поддерживает упрощённую систему ООП, а именно: [классы](#классы), [поля/методы классов](#поля-и-методы), [контекст `this`](#контекст-this), [конструкторы](#конструкторы), [создание экземпляров классов](#экземпляры), [вызов полей/методов из экземпляров](#вызовы-членов-класса).
...
### Экземпляры
Экземпляр класса `<класс>` - переменная, с типом `<класс>` и выражением в виде `new <класс>(<аргументы_конструктора>)`. Экземпляры классов можно создавать как в основной программе, так и внутри других классов.
...
> [!IMPORTANT]
> Тип перечисляемых аргументов и ожидаемых аргументов конструктора должны быть явно или неявно равны.
> Количество перечисляемых и ожидаемых аргументов должно совпадать.
...
Язык будет дорабатываться, увеличиваться и обрастать новыми возможностями, совсем скоро...
